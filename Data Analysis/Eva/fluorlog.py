...
def normalize_max2(data):
    ...
    return [val / max_val for val in data]
# Data inlezen
wavelength1, intensity1 = read_csv('PSI/FLUOROLOG DATA.csv', 0,5 )
wavelength2, intensity2 = read_csv('PSI/FLUOROLOG DATA.csv',0, 2)
...
plt.plot(wavelength1, intensity11, label='Emissie AT')
plt.plot(wavelength2, intensity22, label='Emissie CE')
#  plt.plot(wavelength_common, intensity_diff, label='Difference (AT - CE)', linestyle='--', color='red')
plt.xlabel('Wavelength (nm)')
plt.ylabel('Intensity(A.U.)')
plt.title('Emissie PSI AT/CE LT ')
